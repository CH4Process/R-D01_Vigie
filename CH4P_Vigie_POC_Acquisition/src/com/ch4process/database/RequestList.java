...
public class RequestList
{
	...
	public static final String REQUEST_LogEvent = "INSERT INTO evenement (scenario_id, date) VALUES (?,?);";
	
	public static final String REQUEST_WeekMeasures = "SELECT m.capteur_id, m.valeur, m.datetime, c.libelle, tc.unite, tc.coeff FROM mesure m, capteur c, type_capteur tc WHERE tc.type_capteur_id = c.type_capteur_id AND m.capteur_id = c.capteur_id AND m.datetime >= ?;";
	public static final String REQUEST_Indexes = "SELECT t.capteur_id, c.libelle, t.valeur, t.lastupdate FROM totalizer t, capteur c WHERE c.capteur_id = t.capteur_id;";
	public static final String REQUEST_UpdateIndexes = "UPDATE index i SET i.valeur = ?, i.lastupdate = NOW() WHERE i.capteur_id = ?;";
	public static final String REQUEST_Scenarios = "SELECT s.libelle, s.test, s.params, e.date FROM scenario s, evenement e WHERE e.scenario_id = s.scenario_id;";
}
